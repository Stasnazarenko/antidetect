...
function normalizeProfile(p) {
  ...
  if (copy.proxy && typeof copy.proxy === 'object') {
    if (copy.proxy.server && String(copy.proxy.server).includes('[object Object]')) {
      ...
      if (copy.proxyInfo && copy.proxyInfo.host && copy.proxyInfo.port) {
        copy.proxy = {
          ...
          username: copy.proxy.username || copy.proxy.user || undefined,
    ...
    // Normalize empty server stored as '""' or empty
    if (copy.proxy.server && (String(copy.proxy.server).trim() === '""' || String(copy.proxy.server).trim() === '')) {
      copy.proxy = false;
      copy.proxyType = 'http';
    }
  }
  ...
  // If proxy is string equal to '""' or empty
  if (copy.proxy && typeof copy.proxy === 'string' && (copy.proxy.trim() === '""' || copy.proxy.trim() === '')) {
    copy.proxy = false;
    copy.proxyType = 'http';
  }
